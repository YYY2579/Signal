...
Signal 是一个本地优先的个人 AI 信息情报桌面应用。它把技术社区、项目趋势和官方博客聚合到同一工作区，并提供全文搜索、稍后阅读、知识库、笔记、AI 搜索、文章洞察和文章思维导图。

当前版本为 `0.1.4`。发布工作流为 Windows x64、macOS Intel、macOS Apple Silicon 和 Linux x64 生成安装包与 SHA-256 校验文件；如果 [GitHub Releases](https://github.com/YYY2579/Signal/releases) 暂无可用版本，请按下文从源码运行。
...
2. 在“设置 > 数据源”中启停同步、调整订阅状态、修改刷新间隔，或添加可验证的 RSS/Atom 来源。GitHub、知乎、CSDN、力扣等常用技术来源默认保留；Reddit 因匿名访问可能返回 HTTP 403，默认关闭。
...
6. AI 搜索会先从本地已采集文章中找证据，再由已配置模型补充稳定背景知识。它不是联网搜索，也不会保证覆盖刚发生的事件、全部网页或付费内容。
7. 生成文章洞察前，Signal 会显示待发送的数据并要求确认。默认生成结果保留为草稿，接受后才进入已完成状态。
8. 在文章阅读区打开“文章思维导图”，可基于当前文章的缓存正文或来源摘要生成概念、论点和关系图。生成结果仅保存在本机；选择“重新生成”会替换该文章已有导图。
...
- 结合本地全文命中、近期内容与模型背景知识的 AI 搜索，以及三句话总结、核心观点、影响分析、相关技术、延伸阅读和文章思维导图
