...
- 新增文章 AI 思维导图，可按需生成、交互查看、本地保存和重新生成。
- 新增混合 AI 搜索回答范围与引用统计；本地零命中时仍可返回明确标注的模型知识回答。
- 新增掘金和知乎来源登录卡片，提供站点入口、分步 Cookie 获取提示、保存状态与清除操作。

### Changed

- 工作台导航会取消待执行搜索并清空搜索结果，首页、热门趋势、订阅、摘要、稍后阅读和知识库可立即恢复各自数据视图。
- AI 搜索界面明确区分本地证据与模型知识，并提示它不执行实时互联网检索。

### Fixed

- SQLite 启用 WAL 和写入等待，并将同一来源的文章与热度快照按批次原子写入，修复启动同步期间偶发的外键与数据库锁错误。
- 来源 Cookie 保存失败会返回真实错误，不再显示误导性的成功状态。
